...
# Abstract
Unlike the classical U-shaped bias-variance tradeoff, we see a second descent in the generalization error when we increase the size of the network beyond the interpolation threshold in modern machine learning. This is coined the double descent behavior, which hints at us that "the larger the model, the better!" In this presentation, I link the double descent behavior of neural network training with that of ridgeless (\\(\lambda \rightarrow 0 \\)) kernel regression via the Neural Tangent Kernel. Along the way, the replica trick is introduced to derive an analytic formula for the generalization error of kernel regression.
...
The \\(\LaTeX\\) code to the slides can also be found [here](https://github.com/WenPerng/NTUEE_Projects/tree/main/Physical_Theory_of_Machine_Learning/NTK_Double_Descent_slides).

The MATLAB code to generate the figure below can be found [here](https://github.com/WenPerng/NTUEE_Projects/blob/main/Physical_Theory_of_Machine_Learning/DoubleDescent.m).

<img src='/images/projects/2025_doubleDescent.png' width="70%">
