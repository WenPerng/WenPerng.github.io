...
\begin{align*}
	...
	\end{matrix}\right] &= \left[\begin{matrix}
		\Gamma(0) & \dot{\Gamma}(0)
	\end{matrix}\right] \cdot \lim_{N\rightarrow\infty}\overrightarrow{\prod_{k=0}^{N-1}} \left(\mathbb{I}_{2k} + \frac{t}{N} \left[\begin{matrix}
		0 & -S(tk/N) \\ \mathbb{I}_{k} & 0
	\end{matrix}\right]\right),
\end{align*}
$$
</p>
...
$$
\begin{align*}
	&\mathbb{I}_{2k} + \frac{t}{N} \left[\begin{matrix}
		0 & -S(tk/N) \\ \mathbb{I}_{k} & 0
	\end{matrix}\right] \\
	&= \mathbb{I}_{2k} + \frac{t}{N} \left[\begin{matrix}
		0 & -\mathrm{e}^{tkA/N} S_0 \mathrm{e}^{-tkA/N} \\ \mathbb{I}_{k} & 0
	\end{matrix}\right] \\
	&= \left[\begin{matrix}
		\mathrm{e}^{tkA/N} & 0 \\ 0 & \mathrm{e}^{tkA/N}
	\end{matrix}\right] \left(\mathbb{I}_{2k} + \frac{t}{N}\left[\begin{matrix}
		0 & -S_0 \\ \mathbb{I}_{k} & 0
...
$$
\begin{align*}
	&\lim_{N\rightarrow \infty} \left(\overrightarrow{\prod_{k=0}^{N-1}} \left(\mathbb{I}_{2k} + \frac{t}{N} \left[\begin{matrix}
		0 & -S_0 \\ \mathbb{I}_{k} & 0
	\end{matrix}\right]\right) \left[\begin{matrix}
	...
	\end{matrix}\right] \\
	&= \lim_{N\rightarrow \infty} \left(\left(\mathbb{I}_{2k} + \frac{t}{N}\left[\begin{matrix}
		0 & -S_0 \\ \mathbb{I}_{k} & 0
	\end{matrix}\right]\right) \left[\begin{matrix}
		\mathbb{I}_k + \frac{tA}{N} & 0 \\ 0 & \mathbb{I}_k + \frac{tA}{N}
	\end{matrix}\right]\right)^N \left[\begin{matrix}
		\mathrm{e}^{-tA} & 0 \\ 0 & \mathrm{e}^{-tA}
	\end{matrix}\right] \\
	&= \lim_{N\rightarrow \infty} \left(\mathbb{I}_{2k} + \frac{t}{N} \left[\begin{matrix}
		A & -S_0 \\ \mathbb{I}_k & A
	\end{matrix}\right]\right)^N \left[\begin{matrix}
		\mathrm{e}^{-tA} & 0 \\ 0 & \mathrm{e}^{-tA}
	\end{matrix}\right] \\
...
The rest of the derivation is the as in the paper and I'll not repeat them here. Hence, we obtain the same result as above.
...
---
A fun and quick journey right? But don't worry, this is not the end. There is more to this paper that I would love to dig deeper into. So stay tuned and wait for "Paper review: The Geoemtry of Algorithms with Orthogonality Constraints (II)"!