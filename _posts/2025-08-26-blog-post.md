...
By applying this lemma and we obtain
<p>

$$
\begin{align*}
	B_{t} &= B_{t-1} + \left[\begin{matrix}
		u & v
	...
	%\end{matrix}\right] \\
	\Rightarrow B_{t}^{-1} &= B_{t-1}^{-1} - B_{t-1}^{-1} \left[\begin{matrix}
		y_{t-1} & B_{t-1}s_{t-1}
	\end{matrix}\right] \ldots \\
	&\hspace{1cm}\left(\left[\begin{matrix}
		s_{t-1}^{\mathsf{T}} y_{t-1} & 0 \\ 0 & s_{t-1}^{\mathsf{T}} B_{t-1} s_{t-1}
	\end{matrix}\right] \ldots \right.\\
	&\hspace{1cm} \left. - \left[\begin{matrix}
